...
+time(CurrentSimTime) : insertTime(T) & targetSpeed(V) & driverAgentName(DAgentName) & scenario(vsl) & CurrentSimTime = T <- .print("can set agent vsl behaviour now");
							.wait(500); 
							setLaneChange(0);					
							.wait(500);
...
+time(CurrentSimTime) : insertTime(T) & targetSpeed(V) & driverAgentName(DAgentName) & scenario(crash) & CurrentSimTime = T <- .print("can set agent crash behaviour now");
...
+time(CurrentSimTime) : insertTime(T) & targetSpeed(V) & driverAgentName(DAgentName) & scenario(flashLights) & CurrentSimTime = T <- .print("can set agent flashLights behaviour now");
							.wait(500); //just give sim second to insert the vehicle..
							//setLaneChange(17);
							setLaneChange(0);
							.wait(500);
							setAutonomy(0);
							.wait(500);
							.print("sending speed message to 31");
							.send(DAgentName, achieve, setSpeed(31)).	

+time(CurrentSimTime) : insertTime(T) & targetSpeed(V) & driverAgentName(DAgentName) & CurrentSimTime = T <- .print("can set agent default behaviour now");
							.wait(500); //just give sim second to insert the vehicle..
							//setLaneChange(17);
							setLaneChange(0);
							.wait(500);
...
+detectionInCollisionZone(Name, Distance) : Distance > 45 & Distance <=69 <- .print("vehicle ",Name," detected near in collision zone!! distance is ", Distance);
													.abolish(collisionVolClear);
													!flashLights(Name).		
...
+!flashLights(Target) : true <- .print("Called plan to flash lights!!");
								setLights(on);
								instEvent(flashLights(centralMember1));
								.wait(2000);
								setLights(off).
