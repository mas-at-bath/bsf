...
public class TransitionSystem {
    ...
    public enum ErrFindOp { Success, NoRelPlan, NoAppPlan } 
    ...
    private void applyRelApplPlRule2(String m) throws JasonException {
        ...
        if (conf.C.SE.trigger.isGoal() && !conf.C.SE.trigger.isMetaEvent()) {
        	...
        	if (conf.C.SE.trigger.getLiteral().getFunctor().equals("obl") != true)
        		confP.step = State.SelEv;
        }
    }
    ...
    private void applyFindOp() throws JasonException {
        ...
        ErrFindOp efo = findFirstOption(conf.C.SE.trigger);
        
        if ((efo != ErrFindOp.Success) && (conf.C.SE.trigger.getOperator() == TEOperator.add) && isNorms(conf.C.SE.trigger)) {
        	//addNormExecutionEvent();
        	Literal lt = conf.C.SE.trigger.getLiteral();
        	if (lt.getArity() == 3) {
        		String nevent = lt.getTerm(0).toString() + "[deadline(" + lt.getTerm(1) + "),priority(" + lt.getTerm(2) + ")]";
        		Literal nlt = Literal.parseLiteral(nevent);
        		if (nlt != null) {
        			Trigger te = new Trigger(TEOperator.add, TEType.achieve, nlt);
        			efo = findFirstOption(te);
        		}
        	}
        }
    ...
    private ErrFindOp findFirstOption(Trigger te) {
        // get all relevant plans for the selected event
        ...
        if (candidateRPs != null) {
            ...
            return ErrFindOp.NoAppPlan;
        } else {
            // problem: no plan
        	return ErrFindOp.NoRelPlan;
        }        
    }
    
    private void postFindOp(ErrFindOp code) throws JasonException {
    	switch (code) {
	    	case NoAppPlan:
	    		applyRelApplPlRule2("applicable");
	    		break;
    ...
    // added by jeehanglee@gmail.com
    private void addNormExecutionEvent() {
    	Literal lt = conf.C.SE.trigger.getLiteral();
    	if (lt.getArity() == 3) {
    		String nevent = lt.getTerm(0).toString() + "[deadline(" + lt.getTerm(1) + "),priority(" + lt.getTerm(2) + ")]";
    		Literal nlt = Literal.parseLiteral(nevent);
    		if (nlt != null) {
    			Trigger te = new Trigger(TEOperator.add, TEType.achieve, nlt);
    			updateEvents(new Event(te, Intention.EmptyInt));
    		}
    	}
    }
    
    private boolean isNorms(Trigger te) {
    	if (te.getLiteral().getFunctor().equals("obl"))
    		return true;
    	return false;
    }
