...
public class Pred extends Structure {
    ...
    @Override
    public boolean hasAnnot(String functor) {
        if (annots != null) {
            for (Term ta : annots) {
                if (ta.isStructure()) {
    ...
    // Extract properties by property names (deadline/priority) 
    // from annotations (by jeehanglee@gmail.com) 
    public double getAnnotsVal(String functor) {
		if (annots != null) {
            for (Term ta : annots) {
                if (ta.isStructure()) {
                    if (((Structure)ta).getFunctor().equals(functor)) {
                    	NumberTerm nt = (NumberTerm) ((Structure)ta).getTerm(0);
                    	if (nt != null)
                    		return nt.solve();
                    }
                }
