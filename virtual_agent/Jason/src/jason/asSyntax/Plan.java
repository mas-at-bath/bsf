...
public class Plan extends Structure implements Cloneable, Serializable {
    ...
    private long duration = 50;		// expected execution time of current plan [20130627, jeehanglee@gmail.com]
    private long priority = 0;		// priority of the current plan. Set 0 as default
    private long deadline = Long.MAX_VALUE;		// deadline of the current plan. Set 0 as default meaning infinite 
    ...
    public Plan(Pred label, Trigger te, LogicalFormula ct, PlanBody bd) {
        super("plan", 0);
        setTEvent(te);		// pop out as an independent method, jeehanglee@gmail.com
        setLabel(label);
        setContext(ct);
        if (bd == null) {
    ...
    public void setTEvent(Trigger te) {
        tevent = te;
        tevent.setAsTriggerTerm(false);

        // Set the trigger event and extract deadline/priority info, from annotations 
        // by jeehanglee@gmail.com
        Literal lt = te.getLiteral();
        if (lt != null && lt.isPred()) {
        	Pred pred = (Pred) lt;
        	if (pred.hasAnnot("deadline"))
        		this.deadline = (long)((Pred)lt).getAnnotsVal("deadline");
        	if (pred.hasAnnot("priority"))
        		this.priority = (long)((Pred)lt).getAnnotsVal("priority");
        }
    }
    ...
    public void setLabel(Pred p) {
        ...
        if (p != null && p.hasAnnot()) {
            ...
            // Extract duration property from the plan label (by jeehanglee@gmail.com)
            duration = (long)p.getAnnotsVal("duration");
        }
    }
