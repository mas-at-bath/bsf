...
public class PlanLibrary implements Iterable<Plan> {
    ...
    public List<Plan> getCandidatePlans(Trigger te) {
        ...
        if ((l == null || l.isEmpty()) && !varPlans.isEmpty()) {  // no rel plan, try varPlan
        ...
        } else if ((l == null || l.isEmpty()) && varPlans.isEmpty() && (te.getOperator() == TEOperator.add) && isNorm(te)) {	// added by jeehanglee@gmail.com
        	...
        	if (lt.getArity() == 3) {
        		...
        		if (nlt != null) {
        			Trigger nte = new Trigger(TEOperator.add, TEType.achieve, nlt);
        			l = relPlans.get(nte.getPredicateIndicator());
        		}
        	}
        }
    ...
    private boolean isNorm(Trigger te) {
    	return te.getLiteral().getFunctor().equals("obl");
    }
