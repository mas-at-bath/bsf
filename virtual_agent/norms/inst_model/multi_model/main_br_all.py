...
argparser.add_argument("-m", "--mode-option", type=arb, help="specify the mode, single(s/S) or composite(c/C)")
# JAP 20121124
...
iparser = instalparser_v2.makeInstalParser()
...
if args.instal_files_list:
    for name in args.instal_files_list:
        parser= instalparser_v2.makeInstalParser()
        f = open(name,'r')
        document = ""
