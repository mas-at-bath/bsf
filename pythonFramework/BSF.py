...
#needs fromRDF to be defined to start working on Python handling receipt of BSF RDFs
...
class DataReading:
	...
	g.add((aref, URIRef(Constant.sISDATAREADING), URIRef(self.takenBy)))
        g.add((aref, URIRef(Constant.sTAKEN_AT), Literal(self.timestamp)))
       # g.add((aref, URIRef("rdf:type"), Literal(self.type_)))
        g.add((aref, URIRef(Constant.sPHYSICALLOCATION), URIRef(self.locatedAt)))
